...
<div align="center">
  ...
  <br>
  <em>Segmented blood vessels of an input volume using UNETR model🤯</em>
</div>
...
We have tested the code locally on M1 Macbooks (which is really slow, because some models contain PyTorch modules that are not implemented for mps-devices, and hence only CPU acceleration is possible), and we were not able to execute the code on either Google Colab or Google Cloud - keep this in mind if you want to run the code yourself.
### 🚂 Training 🚂
